...
class Weekly(commands.Cog):
    ...
    async def seed(self, ctx, *args):
        ...
        weekly = self.db.get_open_weekly(game)
        if weekly is None:
        ...
        if entry is None:
            registered = self.db.get_registered_entry(author.id)
            ...
            self.db.register_player(weekly, author.id, get_discord_name(author))
    ...
    async def time(self, ctx, *args):
        ...
        entry = self.db.get_registered_entry(author_id)
        if entry is None:
        ...
        self.db.submit_time(entry.weekly, author_id, time, ctx.message.attachments[0].url)
    ...
    async def forfeit(self, ctx, *args):
        ...
        self.db.forfeit_player(entry.weekly, author_id)
